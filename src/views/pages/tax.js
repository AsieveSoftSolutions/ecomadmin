...
const Tax = () => {
    ...
    function editBtn(itemList) {
        try {
            ...
            setTaxName(itemList.tax_name);
            setTaxPercentage(itemList.tax_percentage)
            document.getElementById("category_checkbox").checked = itemList.is_active == 1 ? true : false;
            setHeaderTiltle("Edit Tax");
            setButtonName("Update");
            setErrorMsgText('');
            setGridDisplay('None');
            setAddDisplay("block");
                ...
                .then(function (response) {
                    ...
                    if (response.data.error_code == '9999') {
                        ...
                        if (response.data.data[0].data.length > 0) {
                            setTaxList(response.data.data[0].data);
        ...
        try {
            ...
            if (validate) {
                if (sessionStorage.getItem("edit_data") == null) {
                    ...
                    let postData = {
                        "tax_id": "0",
                        "tax_name": taxNameRef.current.trim(),
                        "tax_percentage":taxPercentageRef.current,
                    ...
                    axios({
                        method: "put",
                        url: Apipath['UpdateTax'] + data.tax_id
            ,
                        headers: {
                            // 'Authorization': `bearer ${token}`,
                            'Content-Type': 'application/json'
    ...
    function validation() {
        try {
            ...
            else if(taxPercentageRef.current == '0'){
                    ...
                    <div className='table_card my-3 mx-auto'>
                        <div className="row  justify-content-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3 text-end">
                                <button className='addBtn' onClick={() => addBtn()}>Add Tax<img src={addicon} className='action_icon ms-2'></img></button>
                            </div>
                        </div>
                        <div className="row  justify-content-end align-items-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3" >
                                ...
                                <select name="cars" id="filter_active_dropdown" className='input_tag' value={selectActiveId}
                                    onChange={(e) => { setSelectActiveId(e.target.value); setCurrentPage(1); getTaxList() }}>
                            ...
                            <div className=" mt-1 col-sm-12 col-md-6 col-lg-4 col-xl-4">
                                <div className=' align-items-center input_div'>
                                    <input type='search' placeholder='Search...' className='input_box' id='search'
                                        onChange={() => { setCurrentPage(1); getTaxList(); }} />
                                        ...
                                        taxList.map((item, index) => (
                                            <tr>
                                                ...
                                                <td><input type='checkbox'
                                                    checked={item.is_active == 1 ? true : false} onClick={() => activeOnChange(item.is_active, item.tax_id)} />
                            ...
                            <div className="col-sm-12 mt-1 col-md-8 col-lg-6 col-xl-6">
                                <div className='row justify-content-end'>
                                    ...
                                    <div className='col-xl-12 mt-2'>
                                        <label className='label_text'>Tax Name</label>
                                        <input type='text' placeholder=' Enter' className='input_tag' id="p_service" value={taxName}
                                            onChange={(e) => { setTaxName(e.target.value); setErrorMsgText('') }} />
                                    </div>
                                    <div className='col-xl-12 mt-2'>
                                        <label className='label_text'>Tax Percentage</label>
                                        <input type='number' min={0} placeholder=' Enter' className='input_tag' id="url" value={taxPercentage}
                                            onChange={(e) => { setTaxPercentage(e.target.value); setErrorMsgText('') }} />
                                    </div>
