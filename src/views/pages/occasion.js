...
const Occasion = () => {
    ...
    function getOccasionList(){
        try {
            if (window.GetOccasionList){
                window.GetOccasionList.cancel();
            }
            window.GetOccasionList = axios.CancelToken.source();
            setLoader(true);
            let postData = {
            ...
            axios({
                method: "post",
                url: Apipath['GetOccasionList'],
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
                data: postData,
                cancelToken: window.GetOccasionList.token,
            })
                .then(function (response) {
                    ...
                    if (response.data.error_code == '9999') {
                        ...
                        if (response.data.data[0].data.length > 0) {
                            setOccasionList(response.data.data[0].data);
        ...
        try {
            ...
            if (validate) {
                if (sessionStorage.getItem("edit_data") == null) {
                    ...
                    let postData = {
                        "occasion_id":"0",
                        "occasion_Name": document.getElementById("occasion").value.trim(),
                    ...
                    axios({
                        method: "put",
                        url: Apipath['UpdateOccasion'] + data.occasion_id,
                        headers: {
                            // 'Authorization': `bearer ${token}`,
                            'Content-Type': 'application/json'
                        },
    ...
    function validation() {
        try {
            if (document.getElementById('occasion').value == "") {
                ...
                setErrorMsgText("Please enter occasion field");
                return false;
            }
            else {
                return true;
            ...
            <div>
                <div className='card W-100 p-1' style={{ display: gridDisplay }} >
                    ...
                    <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3 text-end">
                                <button className='addBtn' onClick={() => addBtn()}>Add Occasion<img src={addicon} className='action_icon ms-2'></img></button>
                            </div>
                    </div>
                    <div className="row mt-3 justify-content-end align-items-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3" >
                                ...
                                <select name="cars" id="filter_active_dropdown" className='input_tag' value={selectActiveId}
                                    onChange={(e) => { setSelectActiveId(e.target.value);  setCurrentPage(1); getOccasionList() }}>
                            ...
                            <div className=" mt-1 col-sm-12 col-md-6 col-lg-4 col-xl-4">
                                <div className=' align-items-center input_div'>
                                    ...
                                    onChange={()=> {  setCurrentPage(1);getOccasionList();}} />
                                ...
                                {loader &&
                                    ...
                                    {!loader && occasionList.length > 0 &&
                                        occasionList.map((item, index) => (
                                            <tr>
                                                <td>{index + 1}</td>
                                                <td>{item.occasion_Name}</td>
                                                <td><input type='checkbox'
                                                    checked={item.is_active == 1 ? true : false} onClick={() => activeOnChange(item.is_active, item.occasion_id)} />
                            ...
                            <div className="col-sm-12 mt-1 col-md-8 col-lg-6 col-xl-6">
                                <div className='row justify-content-end'>
                                    ...
                                    <div className='col-xl-12 mt-2'>
                                        ...
                                        <input type='text' placeholder=' Enter' className='input_tag' id="occasion" value={occasion}
                                            onChange={(e) => { setOccasion(e.target.value); setErrorMsgText('') }} />
                                    </div>
