...
const Size = () => {
    ...
    async function addBtn() {
        try {
            ...
            setGridDisplay('None');
            setAddDisplay("block");
            setHeaderTiltle("Add Product Size");
            setButtonName("Save");
    ...
    function getProductSizeList() {
        try {
            if (window.GetProductSizeList) {
                window.GetProductSizeList.cancel();
            }
            window.GetProductSizeList = axios.CancelToken.source();
            setLoader(true);
            let postData = {
            ...
            axios({
                method: "post",
                url: Apipath['GetProductSizeList'],
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
                data: postData,
                cancelToken: window.GetProductSizeList.token,
            })
                .then(function (response) {
                    ...
                    if (response.data.error_code == '9999') {
                        ...
                        if (response.data.data[0].data.length > 0) {
                            setProductSizeList(response.data.data[0].data);
    ...
    function activeOnChange(is_active, id) {
        try {
            ...
            axios({
                method: "put",
                url: Apipath['UpdateProductSize'] + id,
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
        ...
        try {
            ...
            if (validate) {
                if (sessionStorage.getItem("edit_data") == null) {
                    ...
                    let postData = {
                        "size_id": "0",
                        "size_Name": document.getElementById('product_size').value.trim(),
                    ...
                    axios({
                        method: "put",
                        url: Apipath['UpdateProductSize'] + data.size_id,
                        headers: {
                            // 'Authorization': `bearer ${token}`,
                            'Content-Type': 'application/json'
                        },
    ...
    function validation() {
        try {
            if (document.getElementById('product_size').value == "") {
                ...
                setErrorMsgText("Please enter product size field");
                return false;
            }
            else {
                return true;
                    ...
                    <div className='table_card my-3 mx-auto'>
                        <div className="row  justify-content-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3 text-end">
                                <button className='addBtn' onClick={() => addBtn()}>Add Product Size<img src={addicon} className='action_icon ms-2'></img></button>
                            </div>
                        </div>
                        <div className="row justify-content-end align-items-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3" >
                                ...
                                <select name="cars" id="filter_active_dropdown" className='input_tag' value={selectActiveId}
                                    onChange={(e) => { setSelectActiveId(e.target.value); setCurrentPage(1); getProductSizeList() }}>
                            ...
                            <div className=" mt-1 col-sm-12 col-md-6 col-lg-4 col-xl-4">
                                <div className=' align-items-center input_div'>
                                    <input type='search' placeholder='Search...' className='input_box' id='search'
                                        onChange={() => { setCurrentPage(1); getProductSizeList(); }} />
                        ...
                        <div className=" mt-2" style={{ height: "53vh", overflowY: "scroll" }} >
                            <table className=' w-100'>
                                ...
                                <tbody>
                                    ...
                                    {!loader && productSizeList.length == 0 &&
                                        <tr className="text-center" colSpan="16">
                                        ...
                                        productSizeList.map((item, index) => (
                                            <tr>
                                                ...
                                                <td><input type='checkbox'
                                                    checked={item.is_active == 1 ? true : false} onClick={() => activeOnChange(item.is_active, item.size_id)} />
                            ...
                            <div className="col-sm-12 mt-1 col-md-8 col-lg-6 col-xl-6">
                                <div className='row justify-content-end'>
                                    ...
                                    <div className='col-xl-12 mt-2'>
                                        <label className='label_text'>Product Size</label>
                                        <input type='text' placeholder=' Enter' className='input_tag' id="product_size" value={productSize}
                                            onChange={(e) => { setProductSize(e.target.value); setErrorMsgText('') }} />
                                    </div>
