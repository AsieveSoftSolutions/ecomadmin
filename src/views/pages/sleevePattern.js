...
const SleevePattern = () => {
    ...
    const [sleevePatternList, setSleevePatternList] = useState([]);
    ...
    function defaultValue() {
        try {
            ...
            if (sessionStorage.getItem("edit_data") == null) {
                setHeaderTiltle("Add Sleeve Pattern");
                setButtonName("Save");
            }
            else {
                let data = JSON.parse(sessionStorage.getItem("edit_data"))
                setHeaderTiltle("Edit Sleeve Pattern");
    ...
    function editBtn(itemList) {
        try {
            ...
            setSleevePattern(itemList.sleeve_pattern_Name);
    ...
    function getSleevePatternList() {
        try {
            if (window.GetSleevePatternList) {
                window.GetSleevePatternList.cancel();
            }
            window.GetSleevePatternList = axios.CancelToken.source();
            setLoader(true);
            let postData = {
            ...
            axios({
                method: "post",
                url: Apipath['GetSleevePatternList'],
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
                data: postData,
                cancelToken: window.GetSleevePatternList.token,
            })
    ...
    function activeOnChange(is_active, id) {
        try {
            ...
            axios({
                method: "put",
                url: Apipath['UpdateSleevePatternList'] + id,
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
        ...
        try {
            ...
            if (validate) {
                if (sessionStorage.getItem("edit_data") == null) {
                    ...
                    let postData = {
                        "sleeve_pattern_id": "0",
                        "sleeve_pattern_Name": document.getElementById('sleeve_pattern').value.trim(),
                    ...
                    axios({
                        method: "put",
                        url: Apipath['UpdateSleevePatternList'] + data.sleeve_pattern_id,
                        headers: {
                            // 'Authorization': `bearer ${token}`,
                            'Content-Type': 'application/json'
                        },
    ...
    function validation() {
        try {
            if (document.getElementById('sleeve_pattern').value == "") {
                document.getElementById("sleeve_pattern").focus();
                setErrorMsgText("Please enter sleeve pattern field");
                return false;
            }
            else {
                return true;
                    ...
                    <div className='table_card my-3 mx-auto'>
                        <div className="row  justify-content-end ">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3 text-end">
                                <button className='addBtn' onClick={() => addBtn()}>Add Sleeve Pattern<img src={addicon} className='action_icon ms-2'></img></button>
                            </div>
                        </div>
                        <div className="row  justify-content-end align-items-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3" >
                                ...
                                <select name="cars" id="filter_active_dropdown" className='input_tag' value={selectActiveId}
                                    onChange={(e) => { setSelectActiveId(e.target.value); setCurrentPage(1); getSleevePatternList() }}>
                            ...
                            <div className=" mt-1 col-sm-12 col-md-6 col-lg-4 col-xl-4">
                                <div className=' align-items-center input_div'>
                                    <input type='search' placeholder='Search...' className='input_box' id='search'
                                        onChange={() => { setCurrentPage(1); getSleevePatternList(); }} />
                        ...
                        <div className=" mt-2" style={{ height: "53vh", overflowY: "scroll" }} >
                            <table className=' w-100'>
                                ...
                                <tbody>
                                    ...
                                    {!loader && sleevePatternList.length == 0 &&
                                        <tr className="text-center" colSpan="16">
                                            <th colSpan="8" className="nofount_text" > No Data Found </th> </tr>}
                                    {!loader && sleevePatternList.length > 0 &&
                                        sleevePatternList.map((item, index) => (
                                            <tr>
                                                <td>{index + 1}</td>
                                                <td>{item.sleeve_pattern_Name}</td>
                                                <td><input type='checkbox'
                                                    checked={item.is_active == 1 ? true : false} onClick={() => activeOnChange(item.is_active, item.sleeve_pattern_id)} />
                            ...
                            <div className="col-sm-12 mt-1 col-md-8 col-lg-6 col-xl-6">
                                <div className='row justify-content-end'>
                                    ...
                                    <div className='col-xl-12 mt-2'>
                                        ...
                                        <input type='text' placeholder=' Enter' className='input_tag' id="sleeve_pattern" value={sleevePattern}
                                            onChange={(e) => { setSleevePattern(e.target.value); setErrorMsgText('') }} />
                                    </div>
