...
const NeckDesign = () => {
    ...
    const [neckDesignList, setNeckDesignList] = useState([]);
    ...
    function defaultValue() {
        try {
            ...
            if (sessionStorage.getItem("edit_data") == null) {
                setHeaderTiltle("Add Neck Design");
                setButtonName("Save");
            }
            else {
                let data = JSON.parse(sessionStorage.getItem("edit_data"))
                setHeaderTiltle("Edit Neck Design");
    ...
    function editBtn(itemList) {
        try {
            ...
            setNeckDesign(itemList.neck_design_Name);
    ...
    function getNeckDesignList() {
        try {
            if (window.GetNeckDesignList) {
                window.GetNeckDesignList.cancel();
            }
            window.GetNeckDesignList = axios.CancelToken.source();
            setLoader(true);
            let postData = {
            ...
            axios({
                method: "post",
                url: Apipath['GetNeckDesignList'],
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
                data: postData,
                cancelToken: window.GetNeckDesignList.token,
            })
                .then(function (response) {
                    setLoader(false);
                    if (response.data.error_code == '9999') {
                        if (response.data.data[0].data.length > 0) {
                            setNeckDesignList(response.data.data[0].data);
    ...
    function activeOnChange(is_active, id) {
        try {
            ...
            axios({
                method: "put",
                url: Apipath['UpdateNeckDesign'] + id,
                headers: {
                    // 'Authorization': `bearer ${token}`,
                    'Content-Type': 'application/json'
                },
        ...
        try {
            ...
            if (validate) {
                if (sessionStorage.getItem("edit_data") == null) {
                    ...
                    let postData = {
                        "neck_design_id": "0",
                        "neck_design_Name": document.getElementById("neck_design").value.trim(),
                    ...
                    axios({
                        method: "put",
                        url: Apipath['UpdateNeckDesign'] + data.neck_design_id,
                        headers: {
                            // 'Authorization': `bearer ${token}`,
                            'Content-Type': 'application/json'
                        },
    ...
    function validation() {
        try {
            if (document.getElementById('neck_design').value == "") {
                document.getElementById("neck_design").focus();
                setErrorMsgText("Please enter neck Design  field");
                return false;
            }
            else {
                return true;
                    ...
                    <div className='table_card my-3 mx-auto'>
                        <div className="row  justify-content-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3 text-end">
                                <button className='addBtn' onClick={() => addBtn()}>Add Neck Design<img src={addicon} className='action_icon ms-2'></img></button>
                            </div>
                        </div>
                        <div className="row  justify-content-end align-items-end">
                            <div className="col-sm-6 mt-1 col-md-4 col-lg-3 col-xl-3" >
                                ...
                                <select name="cars" id="filter_active_dropdown" className='input_tag' value={selectActiveId}
                                    onChange={(e) => { setSelectActiveId(e.target.value); setCurrentPage(1); getNeckDesignList() }}>
                            ...
                            <div className=" mt-1 col-sm-12 col-md-6 col-lg-4 col-xl-4">
                                <div className=' align-items-center input_div'>
                                    <input type='search' placeholder='Search...' className='input_box' id='search'
                                        onChange={() => {setCurrentPage(1); getNeckDesignList(); }} />
                        ...
                        <div className=" mt-2" style={{ height: "53vh", overflowY: "scroll" }} >
                            <table className=' w-100'>
                                ...
                                <tbody>
                                    ...
                                    {!loader && neckDesignList.length == 0 &&
                                        <tr className="text-center" colSpan="16">
                                            <th colSpan="8" className="nofount_text" > No Data Found </th> </tr>}
                                    {!loader && neckDesignList.length > 0 &&
                                        neckDesignList.map((item, index) => (
                                            <tr>
                                                <td>{index + 1}</td>
                                                <td>{item.neck_design_Name}</td>
                                                <td><input type='checkbox'
                                                    checked={item.is_active == 1 ? true : false} onClick={() => activeOnChange(item.is_active, item.neck_design_id)} />
                            ...
                            <div className="col-sm-12 mt-1 col-md-8 col-lg-6 col-xl-6">
                                <div className='row justify-content-end'>
                                    ...
                                    <div className='col-xl-12 mt-2'>
                                        ...
                                        <input type='text' placeholder=' Enter' className='input_tag' id="neck_design" value={neckDesign}
                                            onChange={(e) => { setNeckDesign(e.target.value); setErrorMsgText('') }} />
                                    </div>
